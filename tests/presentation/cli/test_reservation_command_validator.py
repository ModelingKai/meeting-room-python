...
class TestCliReservationCommandValidator:
    ...
    @pytest.fixture
    def valid_user_raw_input(self) -> CliUserInput:
        return CliUserInput(date='20200516',
                            start_time='1100',
                            end_time='1200',
                            meeting_room_id='A',
                            reserver_id='Bob',
    ...
    def test_入力に不備がない場合はこうなる(self, valid_user_raw_input: CliUserInput):
        expected = CliValidationResult(messages=[])

        assert expected == CliReservationCommandValidator.validate(valid_user_raw_input)

    def test_未入力は許さない_日付(self, valid_user_raw_input: CliUserInput):
    ...
    def test_未入力は許さない_2つ未入力があるとき_日付と終了時刻(self, valid_user_raw_input: CliUserInput):
