...
@freezegun.freeze_time('2020-4-1 10:00')
def test_予約時間帯のかぶりを検知できる_予約時間帯が完全に一致しているパターン():
    r1_1000_1200 = 予約時間帯(使用日時(2020, 4, 1, 10, 00), 使用日時(2020, 4, 1, 12, 00))
    r2_1000_1200 = 予約時間帯(使用日時(2020, 4, 1, 10, 00), 使用日時(2020, 4, 1, 12, 00))

    assert r1_1000_1200.is_overlap(r2_1000_1200)


@freezegun.freeze_time('2020-4-1 10:00')
def test_予約時間帯のかぶりを検知できる_予約時間帯が完全に一致しているパターン():
    r1_1000_1200 = 予約時間帯(使用日時(2020, 4, 1, 10, 00), 使用日時(2020, 4, 1, 12, 00))
    r2_1100_1300 = 予約時間帯(使用日時(2020, 4, 1, 11, 00), 使用日時(2020, 4, 1, 13, 00))

    assert r1_1000_1200.is_overlap(r2_1100_1300)
